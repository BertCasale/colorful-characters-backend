...
const searchCharacters = async (searchTerm) => {
  try {
    const searchedCharacter = await db.any(
      "SELECT characters.*, games.name AS game FROM characters LEFT JOIN games ON characters.game_id=games.id WHERE characters.name ILIKE $1 OR lgbt_type ILIKE $1 OR poc_type ILIKE $1 OR disability_type ILIKE $1",
      searchTerm
    );
